...
function analyzeAudioBuffer(data, sampleRate, channels, numSamples) {
    ...
    const expectedSamples = numSamples * channels;
    ...
    for (let i = 0; i < float32Array.length; i++) {
        ...
        if (i === 0 || absSample < analysis.minAmplitude) analysis.minAmplitude = absSample;
        sum += absSample;
        
        // Считаем нули
        if (absSample === 0) {
            analysis.zeroCount++;
        } else if (absSample > 0.00001) {
    ...
    analysis.avgAmplitude = float32Array.length > 0 ? sum / float32Array.length : 0;
    ...
    let isSine = checkForSineWave(float32Array);
    if (isSine) {
...
function printDetailedReport() {
    ...
    console.log(`  • Фреймов с данными: ${stats.nonZeroFrames} (${(stats.nonZeroFrames/stats.totalFrames*100).toFixed(1)}%)`);
    console.log(`  • Пустых фреймов: ${stats.zeroFrames} (${(stats.zeroFrames/stats.totalFrames*100).toFixed(1)}%)`);
    ...
    if (total > 0) {
        console.log(`  • Нулевые (= 0): ${stats.amplitudeRanges.zero} (${(stats.amplitudeRanges.zero/total*100).toFixed(1)}%)`);
        console.log(`  • Крайне тихие (< 0.0001): ${stats.amplitudeRanges.tiny} (${(stats.amplitudeRanges.tiny/total*100).toFixed(1)}%)`);
        console.log(`  • Тихие (0.0001-0.01): ${stats.amplitudeRanges.quiet} (${(stats.amplitudeRanges.quiet/total*100).toFixed(1)}%)`);
        console.log(`  • Нормальные (0.01-0.1): ${stats.amplitudeRanges.normal} (${(stats.amplitudeRanges.normal/total*100).toFixed(1)}%)`);
        console.log(`  • Громкие (0.1-0.5): ${stats.amplitudeRanges.loud} (${(stats.amplitudeRanges.loud/total*100).toFixed(1)}%)`);
        console.log(`  • Очень громкие (>= 0.5): ${stats.amplitudeRanges.veryLoud} (${(stats.amplitudeRanges.veryLoud/total*100).toFixed(1)}%)`);
    }
    ...
    if (stats.totalFrames === 0) {
        ...
        console.log('  ✅ ПЛАГИН РАБОТАЕТ - получены данные с ненулевой амплитудой');
        console.log(`  📊 Качество сигнала: ${(stats.nonZeroFrames/stats.totalFrames*100).toFixed(1)}% фреймов содержат данные`);
    } else {
        console.log('  ⚠️ Неопределенное состояние - требуется дополнительная диагностика');
    }
}
...
function startDiagnostic() {
    ...
    nativeCapture.setWebRTCAudioCallback((audioData) => {
        ...
        stats.avgAmplitude = (stats.avgAmplitude * (stats.totalFrames - 1) + analysis.avgAmplitude) / stats.totalFrames;
        
        // Выводим информацию о первых 5 фреймах детально
        if (frameCounter <= 5) {
            console.log(`\n━━━ ФРЕЙМ #${frameCounter} (ДЕТАЛЬНЫЙ АНАЛИЗ) ━━━`);
            console.log(`Источник: ${audioData.applicationName || audioData.source || 'unknown'}`);
            console.log(`Параметры: ${audioData.sampleRate}Hz, ${audioData.channels}ch, ${audioData.numSamples} samples`);
            console.log(`Размер данных: ${analysis.byteLength} байт (ожидалось: ${analysis.expectedBytes})`);
            console.log(`Амплитуды: max=${analysis.maxAmplitude.toFixed(6)}, avg=${analysis.avgAmplitude.toFixed(6)}`);
            console.log(`Ненулевых сэмплов: ${analysis.nonZeroCount}/${analysis.actualSamples}`);
            console.log(`Паттерн: ${analysis.patternType}`);
            
            if (analysis.firstSamples.length > 0) {
                console.log(`Первые сэмплы: [${analysis.firstSamples.slice(0, 5).map(s => s.toFixed(4)).join(', ')}...]`);
            }
        ...
        if (frameCounter % 100 === 0) {
            ...
            console.log(`   • С данными: ${stats.nonZeroFrames} (${(stats.nonZeroFrames/stats.totalFrames*100).toFixed(1)}%)`);
            console.log(`   • Пустых: ${stats.zeroFrames} (${(stats.zeroFrames/stats.totalFrames*100).toFixed(1)}%)`);
