...
export class AudioProcessor {
  ...
  decodeWindowsAudio(
    ...
    if (arrayBuffer.byteLength === samples * channels * 4) {
      ...
      for (let i = 0; i < Math.min(100, samples); i++) {
        if (
          Math.abs(leftChannel[i]) > 0.00001 ||
          Math.abs(rightChannel[i]) > 0.00001
        ) {
          hasData = true;
          break;
        }
  ...
  normalizeAudio(
    ...
    if (levels.hasAudio) {
      ...
      const gain =
        currentPeak > 0.001 ? Math.min(targetPeak / currentPeak, 3.0) : 1.0;
 ...
 */
export function getSendAudioToJitsiCode(
