...
export class AudioSessionService {
    ...
    try {
      ...
      if (svvPath) {
        ...
        if (hasVc) {
          ...
          log.warn("[AudioSession] VB-Cable NOT found via SVV");
        }
      } else {
        log.warn(
          "[AudioSession] SoundVolumeView not found - audio routing disabled",
        );
      }
  ...
  async isReady(): Promise<boolean> {
    return this.svvLocator.isReady();
  }
  ...
  private async getSVVPath(): Promise<string | null> {
    return this.svvLocator.findSVV();
  }
  ...
  private parseCSV(csv: string): AudioSession[] {
    ...
    for (const line of dataLines) {
      ...
      if (type === "Application" && direction === "Render") {
        const processId = Number.parseInt(columns[3]?.trim() || "0", 10);
        const processPath = columns[4]?.trim() || "";
        const volume = Number.parseInt(columns[5]?.trim() || "0", 10);
    ...
    for (const line of dataLines) {
      ...
      if (type === "Device" && direction === "Render") {
        ...
        if (name && deviceName) {
          ...
          log.info(
            `[AudioSession] Device: "${name}" -> DeviceName: "${deviceName}" (default: ${isDefault})`,
          );
        }
      }
    }
   ...
   */
  async setAppAudioDevice(
    processPathOrName: string,
    deviceName: string,
  ): Promise<boolean> {
    log.info(
      `[AudioSession] Routing "${processPathOrName}" -> "${deviceName}"`,
    );
    ...
    try {
      ...
      log.info(
        `[AudioSession] Executing: /SetAppDefault "${deviceName}" all "${processPathOrName}"`,
      );
      await execFileAsync(
        svvPath,
  ...
  async restoreDefaultDevice(processPathOrName: string): Promise<boolean> {
    log.info(
      `[AudioSession] Restoring "${processPathOrName}" to default device...`,
    );
    ...
    try {
      ...
      if (!defaultDevice) {
        log.error(
          "[AudioSession] No default device found (all devices are virtual?)",
        );
        log.info(
          "[AudioSession] Available devices:",
          devices.map((d) => `${d.name} -> ${d.deviceName}`),
        );
        throw new Error("No default device found");
      }

      log.info(
        `[AudioSession] Restoring to: "${defaultDevice.name}" (DeviceName: "${defaultDevice.deviceName}")`,
      );

      // ВАЖНО: Используем deviceName для устройства и processPath для приложения!
      await execFileAsync(
        svvPath,
  ...
  async getVBCableDeviceName(): Promise<string | null> {
    try {
      ...
      if (vcDevice) {
        log.info(
          `[AudioSession] Found VB-Cable: "${vcDevice.name}" -> DeviceName: "${vcDevice.deviceName}"`,
        );
