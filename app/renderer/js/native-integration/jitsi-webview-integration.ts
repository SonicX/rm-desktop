...
/* eslint-disable @typescript-eslint/consistent-type-definitions, n/no-unsupported-features/node-builtins, @typescript-eslint/no-unsafe-assignment, @typescript-eslint/no-unsafe-argument, @typescript-eslint/no-unsafe-return, @typescript-eslint/no-unsafe-call, prefer-rest-params, @typescript-eslint/prefer-nullish-coalescing, @typescript-eslint/prefer-optional-chain */

import {MacOSNativeCaptureIntegration} from "./macos-native-capture.js";
...
function setupNativeCapture(): void {
  ...
  window._originalGetUserMedia = navigator.mediaDevices.getUserMedia.bind(
  ...
  navigator.mediaDevices.getUserMedia = async function (
    ...
    // Проверяем, запрашивается ли desktop (Electron-specific constraints)
    const videoConstraints = constraints.video as any;
    if (
      videoConstraints &&
      typeof videoConstraints === "object" &&
      "mandatory" in videoConstraints &&
      videoConstraints.mandatory?.chromeMediaSource === "desktop"
    ) {
      const sourceId = videoConstraints.mandatory.chromeMediaSourceId;
      console.log(
        "[JitsiWebView] Desktop capture requested for source:",
        sourceId,
      );
      ...
      if (
        sourceId &&
        (sourceId === "native:system-audio" || shouldUseNativeCapture(sourceId))
      ) {
        console.log(
          "[JitsiWebView] Using native capture for source:",
          sourceId,
        );
        ...
        try {
          ...
          if (!isAvailable) {
            console.warn(
              "[JitsiWebView] Native capture not available, falling back",
            );
          ...
          const nativeStream =
            await window._nativeCaptureIntegration!.createNativeMediaStream(
              sourceId,
            );
  ...
  if (window.APP?.conference) {
    ...
    window.APP.conference.leave = async function () {
      console.log(
        "[JitsiWebView] Conference leave detected, stopping native capture",
      );
      if (window._nativeCaptureIntegration) {
        await window._nativeCaptureIntegration.stopCapture();
      }

      return Reflect.apply(originalLeave, this, arguments);
    };
  }
  ...
  const observer = new MutationObserver((mutations) => {
    for (const mutation of mutations) {
      for (const node of mutation.addedNodes) {
        if (
          node.nodeType === 1 &&
          node instanceof HTMLElement && // Ищем диалог выбора источника по различным признакам
          (node.classList.contains("desktop-picker") ||
...
function addNativeSourceOption(pickerElement: HTMLElement): void {
  ...
  const sourcesContainer =
    pickerElement.querySelector(".desktop-picker-sources") ||
  ...
  if (!sourcesContainer) {
    console.warn(
      "[JitsiWebView] Sources container not found, trying alternative approach",
    );
...
function addNativeOption(container: Element): void {
  ...
  nativeOption.dataset.sourceId = "native:system-audio";
  ...
  nativeOption.addEventListener("click", () => {
    ...
    const shareButton =
      ...
      document.querySelector(
        'button:not([class*="cancel"]):not([class*="close"])',
      );
