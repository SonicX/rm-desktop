...
export class JitsiNativeStreamIntegration {
  ...
  async createMediaStreamFromNative(): Promise<MediaStream> {
    ...
    try {
      ...
      const scriptProcessor = this.audioContext.createScriptProcessor(4096, 0, 2);
      ...
      this.startFrameUpdates(ctx, audioQueue);
      ...
      console.log('[JitsiNative] MediaStream created with tracks:', tracks.length);
  ...
  async injectIntoJitsi(jitsiWindow: Window) {
    ...
    const originalGetDisplayMedia = jitsiWindow.navigator.mediaDevices.getDisplayMedia;
    const self = this;
    
    // Переопределяем getDisplayMedia
    jitsiWindow.navigator.mediaDevices.getDisplayMedia = async function(constraints?: DisplayMediaStreamConstraints) {
      ...
      try {
        ...
        console.error('[JitsiNative] Failed to create native stream, falling back', error);
      ...
      obtainer.openDesktopPicker = function(options: any, callback: Function) {
        ...
        window.ipcRenderer.invoke('get-capture-status').then((status: any) => {
          if (status.isCapturing && status.currentSourceId) {
            ...
            window.ipcRenderer.invoke('open-source-selector').then((result: any) => {
              if (result.success && result.sourceId) {
                // Запускаем захват
                window.ipcRenderer.invoke('start-native-capture', result.sourceId).then(() => {
